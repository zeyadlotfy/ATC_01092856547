...
import { ShieldCheck, ArrowLeft, Mail, RefreshCw, CheckCircle } from 'lucide-react';
...
export default function VerifyEmailPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (otpString.length !== 6 || !/^\d+$/.test(otpString)) {
            setError(t('user.verifyEmail.invalidOtp'));
            return;
        }

        if (!email) {
            setError(t('user.verifyEmail.emailRequired'));
            return;
        }
        ...
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                setSuccess(true);
                toast.success(t('user.verifyEmail.verificationSuccess'));
                ...
                setError(t('user.verifyEmail.verificationError'));
                toast.error(t('user.verifyEmail.verificationError'));
            }
        } catch (err: any) {
            ...
            if (err.response?.status === 400) {
                setError(t('user.verifyEmail.invalidOtp'));
                toast.error(t('user.verifyEmail.invalidOtp'));
            } else {
                setError(t('user.verifyEmail.serverError'));
                toast.error(t('user.verifyEmail.serverError'));
            }
        } finally {
            setIsLoading(false);
        }
        ...
        <div className="min-h-screen bg-gradient-to-b from-white to-gray-100 dark:from-gray-900 dark:to-gray-800 flex flex-col items-center justify-center py-12 sm:px-6 lg:px-8" dir={isRtl ? 'rtl' : 'ltr'}>
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                ...
                <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
                    {!showEmailForm ?
                        t('user.verifyEmail.subtitle', { email }) :
                        t('user.verifyEmail.enterEmailSubtitle')}
                </p>
            </div>
                    ...
                    {success ? (
                        <div className="text-center">
                            ...
                            <h3 className="mt-2 text-lg font-medium text-gray-900 dark:text-white">
                                {t('user.verifyEmail.verificationSuccess')}
                            </h3>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {t('user.verifyEmail.redirecting')}
                            </p>
                            <div className="mt-4 flex justify-center">
                                <Link href="/login" className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                                    {t('user.verifyEmail.goToLogin')}
                                </Link>
                            </div>
                        </div>
                    ) : showEmailForm ? (
                        <form className="space-y-6" onSubmit={handleEmailSubmit}>
                            ...
                            <div>
                                ...
                                <div className="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input
                                        ...
                                        placeholder={t('user.verifyEmail.emailPlaceholder')}
                                        dir="ltr"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                >
                                    {t('user.verifyEmail.continueButton')}
                                </button>
                            </div>
                        </form>
                    ) : (
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <div className="flex justify-center">
                                <button
                                    ...
                                    className="text-sm font-medium text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 transition-colors duration-300"
                                >
