// filepath: c:\SW\Full-Stack\projects\Areeb-Technology-Competition\project\client\src\components\admin\dashboard\bookings\DeleteConfirmModal.tsx
import React from "react";
import { BookingType } from "@/app/(admin)/admin/dashboard/bookings/page";

interface DeleteConfirmModalProps {
    bookingToDelete: BookingType | null;
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div className="mt-2">
                                    <p className={`text-sm ${isDark ? "text-gray-300" : "text-gray-500"}`}>
                                        {t("bookings.delete.confirmation", {
                                            id: bookingToDelete.id.substring(0, 8)
                                        })}
                                    </p>
                                    <div className={`mt-2 p-3 rounded-md ${isDark ? "bg-gray-700" : "bg-gray-100"}`}>
                                        <p className="text-sm font-medium">{bookingToDelete.event.title}</p>
                                        <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                                            {t(`bookings.statusBooking.${bookingToDelete.status.toLowerCase()}`)}
                                        </p>
                                    </div>
                                    <p className={`text-sm mt-3 ${isDark ? "text-red-400" : "text-red-600"}`}>
                                        {t("bookings.delete.warning")}
                                    </p>
                                </div>
                            </div>
                        </div>
